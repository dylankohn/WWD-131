...
function searchList(list, q) {
    function searchCallback(item) {
      return (
        (item.name && item.name.toLowerCase().includes(q.toLowerCase())) || // Check if `name` exists
        (item.description && item.description.toLowerCase().includes(q.toLowerCase())) || // Check if `description` exists
        (Array.isArray(item.tags) && // Ensure `tags` is an array
          item.tags.some(
            (tag) => tag && typeof tag === "string" && tag.toLowerCase().includes(q.toLowerCase())
          ))
      );
    }
    const filtered = list.filter(searchCallback);
  
    const sorted = filtered.sort((a, b) => parseFloat(a.distance) - parseFloat(b.distance)); // Sort distances as numbers
    return sorted;
  }
